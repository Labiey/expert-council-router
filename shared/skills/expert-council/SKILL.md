...
7. Accept concise successful expert work unless evidence shows it is wrong. Verify important results, then call `expert_feedback` with the `executionId` and verification outcome so local routing can learn. Inspect isolated worktree changes before integration. After integrating or rejecting an isolated mutation result, call `expert_cleanup`; retained worktrees are otherwise removed after the configured retention window.
