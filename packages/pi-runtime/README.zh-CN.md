...
- **专家结束不会关掉窗口。** 跟随器在 `delegation_final` 停止滚动，然后等你按键，你可以慢慢往回翻看整段
  过程，再由你决定它何时消失。
- **窗口自身的超时跟随整次委派，而不是一次尝试**（`retry.maxAttempts x 单次尝试的 timeoutMs`，再乘 1.25
  覆盖尝试之间的准备工作，下限 15 分钟，上限为跟随者的 6 小时），所以它不会在被观察的工作仍在运行时
  先过期。旧规则只把单次尝试乘 1.5，于是一次合法跑到第三次尝试的委派，窗口在 37.5 分钟就先死了。
...
| `tool_silence`（不可配置） | - | 当一次尝试用掉 40% 的 `timeoutMs` 却**一次工具调用都没有**时，发一条 `attention` 告知。失败计数器看不见这种情况——正在思考的专家不会报工具错误——而单看耗时又分不清「想得深」和「卡住了」。它只告警：不 nudge、不中止。 |
